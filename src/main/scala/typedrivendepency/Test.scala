...
object Example5TestTF:
  ...
  type Sql = String
  extension (sc: StringContext) def sql(args: Any*): Sql = sc.s(args*)
  ...
  object UserDatabase:
    ...
    def insert[F[_]: Async](using AppContextProvider[UserDatabase[F]])(user: User): F[User] = 
      InAppContext.get[F, UserDatabase[F]].flatMap(_.insert(user))
  ...
  class SubscriptionService[F[_]: InAppContext[(UserDatabase[F], EmailSender)]: Async]:
    def subscribe(user: User): F[Unit] =
      for
        // _ <- UserDatabase.insert(using InAppContext.get[F, UserDatabase[F]])(user)
        _ <- InAppContext.get[F, EmailSender].flatMap(_.sendEmail(user.email, "Fuck", "Fuck "))
